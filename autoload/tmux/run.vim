...
fu s:get_cmd() abort "{{{2
    ...
    let sigil = '[$%]'
    let cmd = getline('.')->matchstr('^\s*'..cml..'\s*'..sigil..'\zs\s.*')

    if empty(cmd)
        if (&ft is# 'vim' || getcwd() is# $HOME..'/wiki/vim') && getline('.') =~# '[^ "#]'
            let cmd = s:get_vim_cmd(cml)
        elseif &ft is# 'python'
            sil! update
            sil call system('python3 -m py_compile '..expand('%:p:S'))
            let cmd = 'cd '..expand('%:p:h')->shellescape()..' && python3 '..expand('%:p:t:S')
        else
    ...
    elseif cmd =~# '\\\s*$'
        ...
        " The trailing space is needed to preserve a possible alignment.{{{
        "
...
fu s:get_vim_cmd(cml) abort "{{{2
    ...
    "     enddef
    "
    " First, we need to automatically surround the code with `vim -Nu ...` and a
    " heredoc.  But how to  determine the start and end of  the code?
    " The start is the current line, as for the end it's the next non-empty line
    " with a lower indentation level (or end of file).
    "}}}
    let [indent, indent1] = matchlist(curline, '^\s*' .. a:cml .. '\(\s*\)')[:1]
    let pat = indent .. '.*' .. '\n'
    if a:cml == ''
        let pat ..= '\%(' .. indent .. '\)\@!'
    else
        let pat ..= '\%(\s*' .. a:cml .. indent1 .. '\)\@!'
    endif
    let [start, end] = [line('.'), search(pat .. '\|\%$')]
    let lines = getline(start, end)
    let cmd = map(lines, {_,v -> substitute(v, indent, '', '')})
    if curline !~# 'EOF[''"]\=$'
        let cmd = [" vim -Nu NONE -S <(cat <<'EOF'"] + cmd + ['EOF', ')']
    endif
    return join(cmd, "\n")
endfu
